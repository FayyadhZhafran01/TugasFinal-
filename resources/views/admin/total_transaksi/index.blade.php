...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container mx-auto px-6 py-8">
        ...
        focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md"> <a href="{{route('admin.total_transaksi.create')}}">
                    TAMBAH</a>
            </button>
            <div class="relative mx-4"> <span class="absolute inset-y-0 left-0 pl-3 flex itemscenter">
                ...
                <form action="{{ route('admin.total_transaksi.index') }}" method="GET"> <input class="form-input w-full
        ...
        <div class=" -mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                <table class="min-w-full table-auto">
                    <thead class=" justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2"> <span class="text-white">TANGGAL</span> </th>
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">KODE PENJUALAN</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                    ...
                    <tbody class="bg-gray-200">
                        @forelse($total_transaksi as $ti)
                        <tr class="border bg-white">
                            <td class="px-16 py-2">{{ $ti->tanggal}}</td>
                            <td class="px-16 py-2">{{ $ti->penjualan->kode_penjualan}}</td>
                            <td class="px-16 py-2">{{ $ti->jumlah}}</td>
                            <td class="px-16 py-2">{{ $ti->ket}}</td>
                            <td class="px-10 py-2 text-center">
                                <a href="{{route('admin.total_transaksi.edit', $ti->id) }}"
                                    class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                <button onClick="destroy(this.id)" id="{{ $ti->id }}"
                ...
                @if ($total_transaksi->hasPages())
                <div class="bg-white p-3">
                    {{ $total_transaksi->links('vendor.pagination.tailwind') }}
                </div>
                @endif
            </div>
        </div>
